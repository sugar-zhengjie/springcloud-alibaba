...
import com.zj.auth.dto.LogoutDTO;
import com.zj.auth.util.JwtUtils;
...
import com.zj.common.entity.Oauth2Token;
...
//    }
    ...
    public Result postAccessToken( @ApiIgnore Principal principal, @ApiIgnore @RequestParam Map<String, String> parameters) throws HttpRequestMethodNotSupportedException {
        ...
        Oauth2Token oauth2Token = Oauth2Token.builder()
                ...
                .exp(oAuth2AccessToken.getExpiration().getTime() / BaseConstant.Number.THOUSAND)
                .refreshToken(refreshToken.getValue())
                .refreshExpiresIn((int) (refreshToken.getExpiration().getTime() / BaseConstant.Number.THOUSAND - System.currentTimeMillis() / BaseConstant.Number.THOUSAND))
                .refreshExp(refreshToken.getExpiration().getTime() / BaseConstant.Number.THOUSAND)
...
//    }
    ...
    public Result logout(HttpServletRequest request, @RequestBody LogoutDTO logoutDTO) {
        ...
        long currentTimeSeconds = System.currentTimeMillis() / BaseConstant.Number.THOUSAND;

        // 将token和refresh_token同时加入黑名单
        String[] tokenArray = new String[BaseConstant.Number.TWO];
        tokenArray[BaseConstant.Number.ZERO] = token.replace(AuthConstant.JWT_TOKEN_PREFIX, "");
        tokenArray[BaseConstant.Number.ONE] = logoutDTO.getRefreshToken();
        for (int i = BaseConstant.Number.ZERO; i < tokenArray.length; i++) {
            ...
            if (exp - currentTimeSeconds > BaseConstant.Number.ZERO) {
